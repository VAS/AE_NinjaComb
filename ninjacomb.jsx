...
for (i = 0; i < selected.length; i++) {
    ...
    //Create an array with this structure: [scene, shot, take, part, extension]
    baseName = item.name.split('.');
    
    //Create the new compositions

    //Check if the clip has not already been added to other comps.
    ...
    // of the same takel (start a new counter from where the last stopped).
    for (var j = (i+1); j < selected.length; j++) {
        var confItem = selected[j];
        var confName = confItem.name.split('.');
        
        //Check if I found another part of the same take.
		if(isSameTake(baseName, confName) ) {
            ...
            //Add the clip to blacklist: it's already in it's comp, I don't have to check it again.
            blackList.push(confItem);
        }
    }
}
